...
public class RandomLinesPerlinNoiseProfile implements PlaceProfile {
    private final Stack<Material> blocks_stack;
    private final NoiseGenerator noise;
    public RandomLinesPerlinNoiseProfile(){
    ...
    public double position_value(Vector3Int position, PositionDecider.PathNode path_node) {
        return Math.pow(30 - noise.noise(position.x, position.y, position.z) * 30, 2) + position.squared_distance(path_node.get_first_path_block());
    }
